...
function renderCompra() {
  ...
  // Stats
  const totalReal = lista.itens.reduce((s, i) => s + (i.precoReal || 0), 0);
  const totalEst = lista.itens.reduce((s, i) => s + (i.precoEst * i.qtd || 0), 0);
...
function criarItemCompra(item) {
  ...
  li.innerHTML = `
    <button class="btn-check ${item.comprado ? 'checked' : ''}" data-id="${item.id}" aria-label="${item.comprado ? 'Desmarcar' : 'Marcar como comprado'}">
